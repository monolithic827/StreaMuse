...
// The host only pushes a new snapshot when something changes (a status transition, a track
// landing), not on a clock. Position would otherwise sit frozen between those events, so playback
// between snapshots is interpolated locally from wall-clock time - exact, because a track always
// plays at the speed it was recorded at.
...
document.getElementById('tb-min').onclick = () => {
  if (pywebviewReady && window.pywebview && typeof window.pywebview.api.minimize_dj === 'function') {
    window.pywebview.api.minimize_dj();
  }
};

document.getElementById('tb-close').onclick = () => {
  if (pywebviewReady && window.pywebview && typeof window.pywebview.api.hide_dj === 'function') {
    window.pywebview.api.hide_dj();
  }
};
